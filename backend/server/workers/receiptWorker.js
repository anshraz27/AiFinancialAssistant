...
const receiptWorker = new Worker(
  ...
  async (job) => {
    ...
    const extractedExpense = await scanReceipt({ receiptImageUrl, mimeType });
    emit(events.RECEIPT_SCAN_COMPLETED, {
      userId,
      jobId: job.id,
      receiptImageUrl,
    });
    return { extractedExpense, receiptImageUrl };
  },
  { connection },
);

receiptWorker.on("active", (job) =>
  emit(events.RECEIPT_SCAN_STARTED, { userId: job.data.userId, jobId: job.id }),
);
receiptWorker.on("failed", (job, error) =>
  emit(events.RECEIPT_SCAN_FAILED, {
...
receiptWorker.on("error", (error) =>
  console.error("Receipt worker error:", error.message),
);
